...
You can use a hook to create more advanced dynamic hostnames than the simple incremented pattern --- `$\{id\}.example.com` --- from the `hostname` property on a policy. This type of hook calculates the correct hostname and returns that hostname as metadata on the task. To do so, it uses a basic counter system that stores how many nodes have bound to a given policy.

### Prerequisites

Ruby must be installed in `$PATH` for the hook script to succeed.
