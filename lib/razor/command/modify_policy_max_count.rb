...
class Razor::Command::ModifyPolicyMaxCount < Razor::Command
  summary "Changes the maximum node count for an existing policy."
  description <<-EOT
Adjusts the maximum node count for a policy.  The new value must be equal to or
greater than the number of nodes currently bound to the policy. The new value can also be
`null` for an "unlimited" count of nodes bound.
  EOT

  example <<-EOT
To set a policy to match an unlimited number of nodes:

    {"name": "example", "max-count": null}

To set a policy to a maximum of 15 nodes:
