...
class Razor::Command::UpdateTagRule < Razor::Command
  summary "Updates the matching rule for an existing tag."
  description <<-EOT
Changes the rule of the given tag to the new rule. The tag will be
reevaluated against all nodes and each node's tag attribute will be updated to
reflect whether the tag now matches or not. Specifically, the tag will be added
...
optional parameter `force` is set to `true`. Otherwise, it will fail.
  ...
  example <<-EOT
To update a tag rule, and force reevaluation:
  ...
  attr 'force', type: :bool, help: _(<<-HELP)
    ...
    This will not change the policy binding of nodes, which may lead to some
    counterintuitive results, such as a node that does *not* match policy
